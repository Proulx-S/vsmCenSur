...
%% Explore time series in transformed space (area)
    ...
    for vs = 1:length(roi{S}.(acq).(task).vessel)
        ...
        % SVD transform on ts
        [U,SS,V] = svd(ts(:,:),'econ','vector'); % time x vox
        Uts = permute(U,[2 1]).*SS;
        Vts = permute(V,[2 1]).*SS;
        Vts = reshape(Vts,[size(Vts,2) size(ts,[2 3 4])]);
        
        % Area/velocity transform on ts
        ...
        zVal = tsOrig(:,zMask);
        tVal = tsOrig(:,tMask);
        AreaTs = ( size(wVal,2).*mean(wVal-mean(tVal,2),2) + size(zVal,2).*mean(zVal-mean(tVal,2),2) ) ./ mean(wVal-mean(tVal,2),2);
        AreaTs = permute(AreaTs,[2 1]);
        VelTs  = permute(mean(wVal,2),[2 1]);
        % figure('WindowStyle','docked');
        % plot(V); hold on

        % Compute transformed resp
        cmdX = {src.afni}; cmdX{end+1} = ['1dcat ' char(fMat)]; [~,cmdout] = system(strjoin(cmdX,newline));
        mat = str2num(cmdout);
        % mat = mat ./ vecnorm(mat,1,1); % normalize each regressor to 1-norm = 1
        baseRegPerRun = (find(mat(end,:),1,'first')-1)./(R-1);
        ...
        % Solve for beta in U = beta * mat'
        betaUts    = Uts    / mat';
        betaAreaTs = AreaTs / mat';
        betaVelTs  = VelTs  / mat';

        UtsResp = betaUts(:,respReg);
        UtsResp = cat(2,zeros(size(UtsResp,1),1),UtsResp,zeros(size(UtsResp,1),1));
        AreaTsResp = betaAreaTs(:,respReg);
        AreaTsResp = cat(2,zeros(size(AreaTsResp,1),1),AreaTsResp,zeros(size(AreaTsResp,1),1));
        VelTsResp = betaVelTs(:,respReg);
        VelTsResp = cat(2,zeros(size(VelTsResp,1),1),VelTsResp,zeros(size(VelTsResp,1),1));
        

        % Transform resp
        base = permute(mean(cat(3,roi{S}.(acq).(task).vessel(vs).im.basePolyRun.im{:}),3),[4 1 2 3]);
        resp = permute(roi{S}.(acq).(task).vessel(vs).im.resp.im,[4 1 2 3]);
        resp = resp + base;
        tt   = permute(((1:size(resp,1))-1).*dsgn.dt,[1 2 3 4]);
        d2Mask = roi{S}.(acq).(task).vessel(vs).polyMask{ismember(roi{S}.(acq).(task).vessel(vs).polyLabel,'dilate2')};
        % SVD transform on resp
        [U,SS,V] = svd(resp(:,tMask|d2Mask),'econ','vector'); % time x vox (excluding those containing other vessels)
        Uresp = permute(U,[2 1]).*SS;
        Vresp = zeros([size(SS,1) size(resp,[2 3 4])]);
        Vresp(:,tMask|d2Mask) = permute(V,[2 1]).*SS;
        % Area/velocity transform on resp
        wVal = resp(:,wMask);
        zVal = resp(:,zMask);
        tVal = resp(:,tMask);
